...
def load_data_from_drive(gdrive_link):
    # Extract file id from drive link (works for both "id=" and "/d/ID/" formats)
...
with st.expander("🔒 Admin: Update Dataset (Google Drive Link)", expanded=True):
    ...
    **Instructions:**  
    ...
    3. Paste the link below and click 'Load Data'.  
    """)
    gdrive_link = st.text_input("🔗 Paste Google Drive CSV shareable link here:", type="default", key="gdrive_link")
    load = st.button("Load Data")

# ---------- LOAD DATA ----------
if gdrive_link and load:
    with st.spinner("Loading data from Google Drive..."):
        df_original = load_data_from_drive(gdrive_link)
...
# ---------- USE DATA IF ALREADY LOADED ----------
if 'df_original' in st.session_state:
    df_original = st.session_state['df_original']

    # --------- DATA CLEANING (AS IN YOUR LOGIC) ----------
    ...
    months = sorted(df_original['Month'].dropna().unique())
    selected_months = all_selectbox("Select Month(s)", months, default_all=True, key="months_sidebar")
    min_date, max_date = df_original['Date'].min(), df_original['Date'].max()
    date_range = st.sidebar.date_input("Select Date Range", [min_date, max_date], key="date_range_sidebar")
    ...
    df_filtered_global = df_original[
        (df_original['Month'].isin(selected_months)) &
        (df_original['Date'] >= date_range[0]) & (df_original['Date'] <= date_range[1])
    ]
    if cities:
    ...
    def analysis_block(analysis_type, df, idx=""):
        ...
        selected_metric = st.selectbox(f"Select Metric", [m for m in metric_options if m in df.columns], key=f"metric_{idx}_{analysis_type}")
        ...
        if analysis_type == "Monthly Revenue Trend":
            ...
            agg_funcs = {col: 'mean' if col.endswith('%') else 'sum' for col in metric_options if col in df.columns}
            ...
            metric = st.selectbox(
                ...
                [m for m in metric_options if m in df.columns],
